<script lang="ts">
	/**
	 * Login page for client-nav test (issue #21 reproduction)
	...
	async function handleSignIn(event: Event) {
		...
		try {
			...
			await goto('/test/client-nav/dashboard');
		} catch (error) {
...
<div class="mx-auto max-w-2xl p-8">
	<h1 class="mb-6 text-2xl font-bold">Client Nav - Login (Issue #21)</h1>
	<p class="mb-6 text-gray-600">
		Signs in then does <code>goto('/test/client-nav/dashboard')</code> (client-side nav).
	</p>
	...
	<div class="space-y-4">
		...
		{#if auth.isAuthenticated}
			<div class="rounded bg-green-100 p-4" data-testid="authenticated-state">
				<p>
					Already signed in. <a href="/test/client-nav/dashboard" class="underline"
						>Go to dashboard</a
					>
				</p>
			</div>
